...
export default function ProductsPageContent() {
  ...
  const handleExport = () => {
    ...
    const rows = filtered.map((p) => [p.name, p.sku, p.category, String(p.stock), String(p.price)]);
    ...
    <div className="flex-1 bg-[#FAF9F6] min-h-screen">
      ...
      <div className="px-4 sm:px-6 lg:px-10 pt-6 sm:pt-8 lg:pt-10 pb-6 flex items-start justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap w-full sm:w-auto">
          {/* Discount button — shows active indicator when a discount is running */}
          <button
            type="button"
            onClick={() => setIsDiscountModalOpen(true)}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border text-sm font-medium transition-colors ${
              discountActive
                ? "border-[#C9A96E] text-[#C9A96E] bg-[#FAF8F3] hover:bg-[#F5F0E6]"
                : "border-gray-200 text-gray-700 bg-white hover:border-gray-300"
            }`}
          >
            <Tag size={15} />
            {discountActive
              ? `DISCOUNT (${discount!.percentage}% ACTIVE)`
              : "SET DISCOUNT"}
          </button>
          <button
            type="button"
      ...
      <div className="px-4 sm:px-6 lg:px-10">
        <div className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-3 flex-wrap">
          ...
          <div className="relative">
            <button
              type="button"
              onClick={() => { setCategoryDropdownOpen((v) => !v); setStockDropdownOpen(false); }}
              ...
              <div className="absolute top-full left-0 mt-1 z-20 bg-white border border-gray-200 rounded-lg shadow-lg py-1 min-w-[180px]">
                ...
                {CATEGORIES.map((c) => (
                  <button
                    key={c.value}
                    type="button"
                    onClick={() => { setCategoryFilter(c.value); setCategoryDropdownOpen(false); }}
                    className="w-full px-4 py-2 text-sm text-left text-gray-700 hover:bg-[#FAF8F3] transition-colors"
                  >
                    {c.label}
                  </button>
          ...
          <div className="relative">
            <button
              type="button"
              onClick={() => { setStockDropdownOpen((v) => !v); setCategoryDropdownOpen(false); }}
              className="flex items-center gap-2 px-4 py-2.5 border border-gray-200 rounded-lg text-sm text-gray-700 bg-white hover:border-gray-300 transition-colors"
            >
              STOCK:{" "}
              <span className="font-medium">
                {stockFilter === "all" ? "ALL STATUS" : stockFilter.replace("-", " ").toUpperCase()}
              </span>
              <ChevronDown size={14} className="text-gray-400" />
            </button>
            {stockDropdownOpen && (
              <div className="absolute top-full left-0 mt-1 z-20 bg-white border border-gray-200 rounded-lg shadow-lg py-1 min-w-[180px]">
                {(["all", "in-stock", "low-stock", "out-of-stock"] as const).map((s) => (
                  <button
                    key={s}
                    type="button"
                    onClick={() => { setStockFilter(s); setStockDropdownOpen(false); }}
                    className="w-full px-4 py-2 text-sm text-left text-gray-700 hover:bg-[#FAF8F3] transition-colors"
                  >
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <div className="min-w-[820px]">
              ...
              {loading ? (
                <div className="px-6 py-12 text-center text-sm text-gray-400">Loading products…</div>
              ) : pageItems.length === 0 ? (
                ...
                pageItems.map((p) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 pr-4">
                      ...
                      />
                      <div>
                        <p className="font-serif text-[15px] text-gray-900 leading-tight">{p.name}</p>
      ...
      <CreateProductModal isOpen={isModalOpen} onClose={closeModal} product={editingProduct} />
      <DiscountModal
