...
// The order-processor/CEO inbox that gets notified for every new confirmed order.
// Set this in your environment (.env.local / hosting provider env vars).
...
// Product thumbnail: primarily product.imageUrls[0], with a couple of
// fallback field names in case an item was assembled differently upstream.
// Always the first image only, per the site's convention.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 6. Send confirmation emails — one to the buyer, one to the order processor/CEO.
    //    These are sent independently: a failure on one (or both) must never fail
    //    the order itself, since the order + cart write above already succeeded.
    ...
    emailJobs.push(
      resend.emails.send({
        ...
        html: buildAdminEmailHtml({
          ...
          orderId: orderRef.id,
          txRef,
          flwRef: verifyData.data?.flw_ref ?? null,
        }),
      })
    );
