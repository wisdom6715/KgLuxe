...
import { collection, onSnapshot, doc, getDoc, updateDoc, deleteDoc, addDoc, serverTimestamp } from "firebase/firestore";
...
export default function CartPage() {
  ...
  useEffect(() => {
    if (user) {
      ...
      const unsub = onSnapshot(cartRef, async (snap) => {
        ...
        for (const item of localItems) {
          ...
          if (existingItem) {
            await updateDoc(doc(cartRef, existingItem.id), { quantity: existingItem.quantity + item.quantity });
          } else {
    ...
    try {
      if (user) {
        ...
        if (nextQty <= 0) {
          ...
          setItems((prev) =>
            prev.map((i) => (i.id === item.id ? { ...i, quantity: Math.min(nextQty, item.stock) } : i))
          );
          toast.success("Quantity updated");
        }
      }
  ...
  // ... (keep the existing edit handlers)
  ...
  const checkout = () => {
    if (user) {
      ...
      router.push("/auth/login?returnUrl=/cart/checkout");
    }
  };
  return (
